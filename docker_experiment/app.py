...
if st.session_state.stage == "intro":
    ...
    with st.container(border=True):
        ...
        st.markdown(
            """
- **Do not share personal information with the LLM.**  
  Do not enter names, email addresses, phone numbers, or other
  information that could identify you or someone else.

- **LLM responses may take a few seconds to appear.**  
  The LLM generates its response in one go rather than displaying
  it word by word. It may take up to approximately 10 seconds
  before a response appears.


- **Complete the experiment independently.**  
  Please do not discuss the questions or your answers with other
  participants while completing the experiment.

- **Do not refresh or close the page during the experiment**,  
  as this may interrupt your session.
            """
        )

    st.write(
        """
During the experiment, you will work on **two problems**.
    ...
    left_column, button_column, right_column = st.columns(
        [1, 2, 1]
    )
